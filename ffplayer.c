...
int main(int argc, char *argv[])
{
    ...
    for (i=0; i<p_fmt_ctx->nb_streams; i++)
    {
        if (p_fmt_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            printf("[info]: avg_frame_rate.num: %d / den: %d \n",
                       p_fmt_ctx->streams[i]->avg_frame_rate.num,
    ...
    screen = SDL_CreateWindow("simple ffplayer", 
                              ...
                              960,//p_codec_ctx->width, 
                              540,//p_codec_ctx->height,
                              SDL_WINDOW_OPENGL
                              );
    ...
    if((p_codec_ctx->width > 1920)||(p_codec_ctx->height > 1080)){
        sdl_rect.w = p_codec_ctx->width/4;
        sdl_rect.h = p_codec_ctx->height/4;
    }else if((p_codec_ctx->width > 960)||(p_codec_ctx->height > 540)){
        sdl_rect.w = p_codec_ctx->width/2;
        sdl_rect.h = p_codec_ctx->height/2;
    }else {
    ...
    while (1)
    {
        ...
        if (sdl_event.type == SDL_USEREVENT_REFRESH)
        {
            ...
            /* printf("data:0x%x, size:%d \n",p_packet->data,p_packet->size);
            for(int i = 0; i < p_packet->size; i++){
                if(i%16 == 0) printf("\n");
                printf("0x%2x ",*(p_packet->data + i));                
            }
            printf("\n"); */
